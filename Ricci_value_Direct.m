...
for x=2:size(g,3)-1
    for y=2:size(g,4)-1
        for ei=1:3
            for ej=1:3
                dgx(ei,ej,1,x,y)=(1/2)*(squeeze(g(ei,ej,x+1,y))-squeeze(g(ei,ej,x-1,y)));
                dgx(ei,ej,2,x,y)=(1/2)*(squeeze(g(ei,ej,x,y+1))-squeeze(g(ei,ej,x,y-1)));
                dgx(ei,ej,3,x,y)=0;
            end
        end
    end
...
figure; surf(RS(9:size(gi_inter,3)-9,9:size(gi_inter,4)-9));
% figure; imagesc(RS);
